...
function Carrito(props) {
  ...
  for (let i = 0; i < props.carrito.length; i++) {
    precio += props.carrito[i].price
  }

  function deleteCart(articulo) {
    props.setCarrito(props.carrito.filter((producto) => { return (producto.title !== articulo) }))
  }
        ...
        <section className="cart">
          ...
          <div className="cart__container container">
            ...
            {props.carrito.map((articulo, index) => {
              return (<Articulo key={index} tipo="cart" deleteCart={deleteCart} imagen={articulo.image} nombre={articulo.title} precio={articulo.price} />)
            })}
            <hr className="cart__divider" />
